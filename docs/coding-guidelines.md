...
| Soft-keyboard input into a session (terminal, remote desktop) | `ui/design/ImeFunnel`: `ImeFunnelField` is the hidden field that holds IME focus, and an `InputTransformation` turns each edit into PTY bytes and reverts the buffer in place. It holds anchors and nothing else — on a login prompt what is typed is a password, and a field's value is `EditableText` in the semantics tree — and because the transformation runs inside the edit, every edit is read against the value the field is resting at. Carrying that reset through a recomposition instead let a keyboard that delivers one keypress as two edits inside a frame (Samsung's number row) double every other digit, and cost a Backspace the keypress after it |
