...
| Soft-keyboard input into a session (terminal, remote desktop) | `ui/design/ImeFunnel`: `ImeFunnelField` is the hidden field that holds IME focus, `ImeFunnel` turns its edits into PTY bytes. It holds anchors and nothing else — on a login prompt what is typed is a password, and a field's value is `EditableText` in the semantics tree — and it reads an edit against what the editor actually held, which is why `resetLanded` is called from the composition rather than inferred. A keyboard that delivers one keypress as two edits inside a frame (Samsung's number row) doubled every other digit before that |
